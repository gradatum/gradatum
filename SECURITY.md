...
**Current supported version** : `v0.6.4` (latest stable). Prior `0.x` tags are end-of-life.
...
- **Known limitations (CAVEATS)**:
  ...
  - **Logout is client-side only**: the studio logout clears localStorage (`localStorage.removeItem('gradatum_studio_jwt_persist')`). A JWT stolen before logout remains valid until it expires (≤ 1 h for studio sessions). Server-side revocation on logout is planned for v0.6.5.
...
_As of v0.6.4. Review this section on each minor release._

Gradatum is designed for local-first, self-hosted deployments. The following data handling properties hold in v0.6.4:
