...
#   --api-key-file PATH  Path vers fichier api-key (défaut : /etc/gradatum/gradatum-mcp.api-key)
...
if [ -r "$API_KEY_FILE" ]; then
    ...
    NEW_KEY=$(gradatum-admin api-key create --owner mcp-stub --scope vault.* 2>/dev/null \
