#!/usr/bin/env bash
#
# publish-stubs-v0.0.2.sh — Publication séquentielle des 26 stubs v0.0.2 sur crates.io.
#
...
#
# Note prerequisite : Le squat v0.0.0 est LIVE (noms réservés). Les stubs v0.0.1 n'ont
# JAMAIS été publiés (token non fourni). La transition v0.0.0 → v0.0.2 est valide
# (semver croissant — cargo publish n'exige pas de version intermédiaire).
#
# Prérequis :
#   - bash scripts/gen-stubs-v0.0.2.sh  (générer les stubs d'abord)
#   - Un token crates.io avec scope "publish-update" (les noms sont déjà réservés v0.0.0)
#     Obtenir sur https://crates.io/me → API Tokens → New Token → scope: publish-update
#
# Usage :
#   export CRATES_IO_TOKEN=<token>
#   bash scripts/publish-stubs-v0.0.2.sh
#
...
#
# Idempotent : si un crate est déjà à v0.0.2, il sera skippé (cargo publish retourne
...
if [[ ! -d "$STUBS_DIR" ]]; then
    echo "ERROR: ${STUBS_DIR} absent. Exécuter d'abord : bash scripts/gen-stubs-v0.0.2.sh" >&2
    exit 1
fi
...
if [[ "$stubs_count" -lt 26 ]]; then
    ...
    echo "       Exécuter : bash scripts/gen-stubs-v0.0.2.sh" >&2
    exit 1
fi
...
echo "==> ${#CRATES[@]} crates à publier en v0.0.2"
...
==> Prêt à publier 26 stubs v0.0.2 sur crates.io.
...
for name in "${CRATES[@]}"; do
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        printf "SKIP (v0.0.2 déjà publiée)\n"
        publish_skip=$((publish_skip + 1))
...
echo
echo "==================================================================="
echo "==> RAPPORT PUBLICATION v0.0.2"
echo "==================================================================="
echo "  Publiés avec succès : ${publish_ok}"
echo "  Skippés (déjà v0.0.2) : ${publish_skip}"
