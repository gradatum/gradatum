#!/usr/bin/env bash
#
# gen-stubs-v0.0.2.sh — Génère les 26 stubs v0.0.2 avec README enrichis (signatures publiques).
#
# Chaque stub est un mini-projet Cargo ISOLÉ dans crates-publish-stubs-v0.0.2/<name>/
...
#
# Changements vs v0.0.1 :
#   - VERSION 0.0.1 → 0.0.2
#   - STATUS_LINE reflète Phase 2.0c-bis (alpha.5 LIVE 2026-05-07, tag v0.1.0-alpha.5)
#   - ROADMAP mis à jour : Phase 2.1 rc.1 + D5 criterion + cohabitation ≥30j
#   - Public API enrichie pour les 4 crates impactés par alpha.5 :
#       gradatum-acl-auth  : ApiKeyStore trait + SqliteApiKeyStore + ApiKey + ApiKeyMaterial
#       gradatum-auth      : Claims.tenant_id (D3-complet) + JwtService.sign() signature
#       gradatum-server    : POST /auth/exchange + ExchangeResponse V2 (5 champs)
#       gradatum-admin     : subcommands api-key {create,list,revoke,rotate} + token issue
#
# Prérequis :
#   Aucun (génération offline, pas de token requis).
#
# Usage:
#   bash scripts/gen-stubs-v0.0.2.sh
#
# Génère : crates-publish-stubs-v0.0.2/<name>/  (Cargo.toml + src/lib.rs|main.rs + README.md)
# Puis utiliser scripts/publish-stubs-v0.0.2.sh pour publier.
#
# Note : v0.0.1 n'a jamais été publié (token non fourni à ce jour).
# La transition v0.0.0 (LIVE squat) → v0.0.2 est valide (semver croissant).

set -euo pipefail

WORKSPACE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STUBS_DIR="${WORKSPACE_ROOT}/crates-publish-stubs-v0.0.2"
...
readonly STATUS_LINE="**Status** : Alpha — placeholder \`v0.0.2\`. Phase 2.0c-bis Auth Path 2 LIVE 2026-05-07 (git tag \`v0.1.0-alpha.5\`). Source code private until \`v1.0\` public release per D5 criterion. See [gradatum.org](https://gradatum.org)."
readonly PART_OF="**Part of [\`gradatum\`](https://crates.io/crates/gradatum)** — Memory backbone for AI agents."
readonly ROADMAP="- Roadmap : Phase 2.0c-bis (alpha.5 LIVE) → Phase 2.1 \`v0.1.0-rc.1\` (migrate-from-v0 + audit JSONL + rate limiting + scopes granulaires) → \`v0.1.0\` public (post-Phase 4 + D5 criterion ≥30j cohabitation runtime)"
...
> JWT signing + verification (Ed25519, audience-scoped, mandatory `kid`, D3-complet `tenant_id`) and token revocation. Alpha.5 adds `JwtService` and `Claims.tenant_id`.

**Status** : Alpha — placeholder `v0.0.2`. Phase 2.0c-bis Auth Path 2 LIVE 2026-05-07 (git tag `v0.1.0-alpha.5`). Source code private until `v1.0` public release per D5 criterion. See [gradatum.org](https://gradatum.org).

**Part of [`gradatum`](https://crates.io/crates/gradatum)** — Memory backbone for AI agents.

## Public API (alpha.5 — D3-complet)

### Modules

```rust
pub mod jwt;        // JWT Ed25519 signing + verification + JwtService + Claims
pub mod revocation; // RevocationStore — in-memory + SQLite-backed jti revocation
```

### Claims (D3-complet — tenant_id mandatory)

```rust
/// JWT claims. `tenant_id` is mandatory in all tokens (D3-complet / AUTH-T7).
pub struct Claims {
    ...
    pub tenant_id: String,      // D3-complet: tenant identifier ("main" by default)
}
```
...
```rust
/// High-level JWT signing + verification service.
/// Holds Ed25519 keypair, kid, and TTL configuration.
pub struct JwtService { ... }

impl JwtService {
    /// Create a JwtService from an Ed25519 keypair file (PEM/DER).
    pub fn new(
        key_path: &Path,
        pub_key_path: &Path,
        kid: String,
        ttl_service_secs: u64,
        ttl_refresh_secs: u64,
    ) -> Result<Self, JwtError>

    /// Ephemeral in-memory keypair (tests only).
    pub fn new_ephemeral() -> Self

    /// Sign a JWT for `sub` with `scopes` and `tenant_id`.
    pub fn sign(
        &self,
        sub: &str,
        scopes: Vec<Scope>,
    ...
    /// Verify a JWT bearer token. Returns Claims on success.
...
pub enum TokenScope {
    Service,   // long-lived (ttl_service_secs, default 24h)
    Refresh,   // shorter (ttl_refresh_secs, default 1h)
}
```
...
/// SQLite-backed RevocationStore (wired in runtime since alpha.5 / AUTH-T6).
pub struct SqliteRevocationStore { ... }

impl SqliteRevocationStore {
    pub async fn init(db_path: &Path) -> Result<Self, AuthError>
...
> Argon2id bearer credential verification, scope enforcement, and API-key store (Auth Path 2 — alpha.5).

**Status** : Alpha — placeholder `v0.0.2`. Phase 2.0c-bis Auth Path 2 LIVE 2026-05-07 (git tag `v0.1.0-alpha.5`). Source code private until `v1.0` public release per D5 criterion. See [gradatum.org](https://gradatum.org).

**Part of [`gradatum`](https://crates.io/crates/gradatum)** — Memory backbone for AI agents.

## Public API (alpha.5 — Auth Path 2)
...
Operator                     Consumer
  api-key create
    -> SqliteApiKeyStore
         generate(256-bit secret, OsRng)
         argon2id hash (m=19456 KiB, t=2, p=1)
         persist row (api_keys table)
         stdout: ak_xxx (displayed ONCE — D8)
...
pub trait ApiKeyStore: Send + Sync {
    /// Create a new API key for `owner` with `scopes` and `tenant_id`.
    /// Returns the cleartext secret (displayed once — D8).
    async fn create(
        &self,
        owner: &str,
        scopes: Vec<Scope>,
    ...
    /// Verify a raw API key (`ak_xxx`). Returns ApiKey metadata on success.
    async fn verify(&self, raw_key: &str) -> Result<ApiKey, ApiKeyError>;

    /// Revoke a key by its prefix.
    async fn revoke(&self, prefix: &str) -> Result<(), ApiKeyError>;

    /// Rotate a key: revoke old + create new atomically. Returns new cleartext.
    async fn rotate(&self, prefix: &str) -> Result<ApiKeyMaterial, ApiKeyError>;

    /// List all keys for `owner` (revoked included).
...
```rust
/// Metadata for an existing API key (secret never stored/returned after creation).
pub struct ApiKey {
    pub id: String,          // ULID
    pub prefix: String,      // "ak_" + 8-char public prefix
    pub owner: String,       // consumer_id
    pub scopes: Vec<Scope>,
    pub tenant_id: String,   // D3-complet multi-tenancy
    pub created_at: DateTime<Utc>,
    pub description: Option<String>,
}

/// Returned once at creation — contains the cleartext secret (D8: displayed once only).
pub struct ApiKeyMaterial {
    pub key: ApiKey,
    pub secret: String,      // ak_<prefix><secret> — shown once, then gone
}
```
...
```rust
/// SQLite-backed ApiKeyStore with argon2id hashing.
pub struct SqliteApiKeyStore { ... }

impl SqliteApiKeyStore {
    pub async fn init(db_path: &Path) -> Result<Self, ApiKeyError>
    pub async fn in_memory() -> Result<Self, ApiKeyError>
}

/// Prefix constant and secret length.
pub const KEY_PREFIX: &str = "ak_";
pub const SECRET_LEN: usize = 32; // 256-bit secret (base62 encoded)
```

### Legacy functions (bearer token hash — pre-alpha.5)

```rust
/// Verify a bearer token against its argon2id hash.
/// Constant-time comparison to prevent timing attacks.
pub fn verify_bearer(token: &str, hash: &str) -> Result<bool, AclAuthError>
...
pub fn enforce_scope(trust: &TrustContext, required: Scope) -> Result<(), AclAuthError>
```
...
- Secrets stored as argon2id hashes only (OWASP 2023 compliant: m=19456 KiB, t=2, p=1).
- `verify()` uses constant-time argon2 comparison (no timing leak).
- CSPRNG `OsRng` for secret generation (128 bits effective entropy).
- API key displayed ONCE at creation (D8 UX, no re-display).
- Revocation check on every `/auth/exchange` call.
...
```bash
bash scripts/install-gradatum-services.sh
```
...
echo "  crates générés"
echo
echo "Prochaine étape : bash scripts/publish-stubs-v0.0.2.sh"
