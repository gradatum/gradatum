...
//! # Auth (F-1 fix)
//!
//! Depuis F-1, tous les endpoints `/api/v1/notes/*` et `/api/v1/vault_downgrade` exigent
//! un bearer JWT valide. `build_with_concrete_index` retourne désormais un token JWT signé
//! avec tenant=main, identity=downgrade-e2e-tester (preset ACL write main/*).
//! Toutes les requêtes de test incluent `Authorization: Bearer <token>`.
...
/// Preset ACL autorisant read+write sur `main/*` — utilisé par tous les tests F-1.
const ACL_ALLOW: &str = r#"
[[consumer]]
identity = "downgrade-e2e-tester"
read_patterns  = ["main/*"]
write_patterns = ["main/*"]
"#;
...
/// Construit un `(Router, AppState, Arc<SqliteIndex>, String)` partageant le MÊME index in-memory.
///
...
///
/// Le `String` retourné est un bearer JWT signé avec le preset ACL_ALLOW (tenant=main,
/// identity=downgrade-e2e-tester, scopes=[read, write]). À inclure dans les requêtes
/// HTTP via `Authorization: Bearer <token>` — obligatoire depuis F-1.
...
async fn vault_downgrade_success_returns_200() {
    let (app, _state, idx, token) = build_with_concrete_index().await;
...
async fn vault_downgrade_nonexistent_returns_404() {
    let (app, _state, _idx, token) = build_with_concrete_index().await;
...
/// Helper : envoie un PATCH /notes/{id} avec un body JSON et un bearer JWT, retourne le StatusCode.
...
async fn patch_add_tags_empty_list_returns_400() {
    let (app, _state, idx, token) = build_with_concrete_index().await;
    let note_id = seed_note(&idx).await;
    let st = patch_status(&app, &note_id, serde_json::json!({"add_tags": []}), &token).await;
...
async fn patch_add_tags_malformed_tag_returns_400() {
    ...
    let st = patch_status(
        &app,
        &note_id,
        serde_json::json!({"add_tags": ["BadTag"]}),
        &token,
    )
    .await;
...
async fn patch_empty_body_returns_400() {
    let (app, _state, idx, token) = build_with_concrete_index().await;
    let note_id = seed_note(&idx).await;
    let st = patch_status(&app, &note_id, serde_json::json!({}), &token).await;
