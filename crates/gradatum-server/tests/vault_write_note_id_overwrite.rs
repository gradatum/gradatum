...
async fn spawn(seed: Option<(&str, &str)>) -> SocketAddr {
...
async fn overwrite_existing_without_sha_returns_409() {
    ...
    let addr = spawn(Some((nid, "corps original"))).await;
...
async fn note_id_with_malformed_sha_returns_400() {
    ...
    let addr = spawn(None).await; // pas besoin de seed : le 400 précède le lookup
    let resp = reqwest::Client::new()
        .post(format!("http://{addr}/api/v1/vault_write"))
        .bearer_auth(SUB)
        .json(&serde_json::json!({
