//! Synchronous note endpoints — thin wrappers over `logic::*_impl`.
//!
...
//! | POST | `/notes/{id}/move` | 204 No Content | Move note to a new locus |
//!
...
use crate::api_v1::logic::{
    err_to_status, move_note_locus_impl, patch_note_impl, vault_downgrade_impl,
};
...
/// `POST /api/v1/vault_downgrade` — thin wrapper sur `logic::vault_downgrade_impl`.
///
...
/// - **401 Unauthorized** — no valid bearer JWT.
/// - **403 Forbidden** — ACL Write denied or cross-tenant mismatch.
...
async fn vault_downgrade(
    ...
    vault_downgrade_impl(&state, &trust, req)
        .await
        .map(Json)
        .map_err(|e| match e {
...
/// `PATCH /api/v1/notes/{id}` — thin wrapper sur `logic::patch_note_impl`.
///
...
/// - **401 Unauthorized** — no valid bearer JWT.
/// - **403 Forbidden** — ACL Write denied.
...
async fn patch_note(
    ...
    // Construire un body sans add_tags pour patch_note_impl (les tags sont gérés séparément).
    let body_for_impl = NoteStatusPatch {
        add_tags: None,
        ..body
    };

    // Déléguer la logique auth+ACL+métier à patch_note_impl.
    patch_note_impl(&state, &trust, &note_id, body_for_impl)
        .await
        .map_err(|e| match e {
            GradatumError::Unauthorized => StatusCode::UNAUTHORIZED,
            GradatumError::Forbidden(_) => StatusCode::FORBIDDEN,
...
/// `POST /api/v1/notes/{id}/move` — thin wrapper sur `logic::move_note_locus_impl`.
///
...
async fn move_note_locus(
    ...
    // index + suppression orphelin). Auth+ACL gérés dans move_note_locus_impl.
    move_note_locus_impl(&state, &trust, &id, locus)
        .await
        .map_err(|e| match e {
            GradatumError::Unauthorized => StatusCode::UNAUTHORIZED,
            GradatumError::Forbidden(_) => StatusCode::FORBIDDEN,
