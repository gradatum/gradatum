...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        //
        // V3 (F-02) — body limit 256 KiB : protège contre DoS (body volumineux ×
        // nombreux wikilinks → RAM worker). Cap cohérent avec les autres endpoints
        // write. Note texte nominale << 256 KiB ; un body plus grand indique une
        // tentative d'abus.
        .route(
            "/vault_write",
            post(write::vault_write).layer(DefaultBodyLimit::max(256 * 1024)),
        )
