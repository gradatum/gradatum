...
//! Test additionnel (7) : ConnectInfo absent → 403 fail-closed (FIX 4).
//!
...
/// (7) ConnectInfo absent → 403 (fail-closed, FIX 4 régression).
///
/// Sans `ConnectInfo` injecté, le warden ne peut pas déterminer l'IP peer.
/// Comportement attendu : rejeter avec 403 (fail-closed) plutôt que de laisser passer.
#[tokio::test]
async fn connect_info_absent_is_rejected() {
    let config = WardenConfig::default();
    let app = make_app(config);

    // Requête sans ConnectInfo injecté (pas d'appel à r.extensions_mut().insert).
    let req_no_ci = Request::builder()
        .uri("/check")
        .method("GET")
        .body(Body::empty())
        .expect("construction requête");
    // NOTE : on n'insère délibérément pas ConnectInfo.

    let resp = app.oneshot(req_no_ci).await.expect("oneshot");
    assert_eq!(
        resp.status(),
        StatusCode::FORBIDDEN,
        "ConnectInfo absent → 403 attendu (fail-closed), obtenu {}",
        resp.status()
    );
}
