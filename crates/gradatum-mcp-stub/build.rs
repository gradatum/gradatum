...
//! Duplication assumée, désormais à **5 occurrences** (server, worker, gateway,
//! engine, mcp-stub) — le seuil de la règle des trois est franchi de longue date.
//!
...
//! ce crate augmenterait la surface publiée au moment d'un tag ; la dette est
//! tracée ailleurs. Ne PAS placer ce script dans un crate lib déjà largement
//! dépendu (`gradatum-core`) : le `rerun-if-changed` sur `HEAD` y déclencherait
//! la recompilation en cascade de toutes les crates dépendantes à chaque commit.
//!
//! Contrôle de dérive : le corps de ce fichier (à partir de `fn main`) doit rester
//! identique à celui du server —
//! `diff <(sed -n '/^fn main/,$p' mcp-stub/build.rs) <(sed -n '/^fn main/,$p' server/build.rs)`.
//! Même exigence pour `worker/build.rs`, `gateway/build.rs` et `engine/build.rs`.
//!
//! # Robustesse
//!
