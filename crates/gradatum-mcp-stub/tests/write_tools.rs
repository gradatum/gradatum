...
/// Vérifie que le MCP stub forward le POST vers `/api/v1/vault_classify`
/// et restitue fidèlement la réponse 501 du serveur.
#[tokio::test]
async fn vault_classify_forwards_to_correct_route() {
    let server = MockServer::start().await;
    ...
    // Le serveur gradatum retourne 501 pour vault_classify en v0.4.x.
    let not_implemented_body = serde_json::json!({
    ...
    Mock::given(method("POST"))
        ...
        .and(header_exists("Authorization"))
        .and(body_json(request_body.clone()))
        .respond_with(ResponseTemplate::new(501).set_body_json(not_implemented_body))
        .expect(1)
        .mount(&server)
        .await;
