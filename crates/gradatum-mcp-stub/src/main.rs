...
/// Builds an MCP tool definition with an `inputSchema` auto-derived from `T`.
///
/// `T` must derive `schemars::JsonSchema` (via the `schemars` feature in `gradatum-dto`).
/// The resulting schema is the exact wire HTTP contract expected by the server.
...
/// Builds a parameter-less MCP tool definition (GET-only endpoints).
///
/// The JSON schema is `{"type":"object","properties":{}}`, which is valid for
/// "no parameters expected". Used for `vault_status`, `vault_authors`, and `vault_tags`.
...
mod tests {
    ...
    fn list_tools_count_matches_expected() {
        ...
        assert_eq!(
            ...
            20,
            "liste canonique doit contenir 20 tools (11 read + 3 write + 4 history F-40 + 1 forget F-44 + 1 lesson recall F-60)"
        );
    }
