//! Google Gemini backend — `POST /v1beta/models/{model}:generateContent`.
//!
//! Uses the Gemini API with:
...
//! ## Response format
//!
//! `candidates[0].content.parts[0].text` contains the response text.
...
/// Google Gemini backend via `generateContent`.
///
/// # Example
///
/// ```rust,no_run
...
impl GeminiCompatBackend {
    /// Creates a Gemini backend targeting the public API with a 30-second timeout.
