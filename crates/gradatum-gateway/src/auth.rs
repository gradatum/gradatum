//! Middleware d'authentification Bearer inbound.
//!
//! Protège tous les endpoints sauf `/health` (doit rester public pour le monitoring).
//!
//! Comportement :
//! - Si `bearer_token` est `None` dans l'`AppState` → pas d'auth, mode local/test.
//! - Si `bearer_token` est `Some(token)` → exige `Authorization: Bearer <token>` sur tous
//!   les endpoints sauf `/health` et les connexions loopback (si `trust_localhost = true`).
//! - `/health` reste TOUJOURS public, quel que soit la configuration.
//! - Bypass loopback : s'appuie sur `ConnectInfo<SocketAddr>.ip().is_loopback()` — adresse
//!   TCP réelle fournie par le kernel. Les headers HTTP (X-Forwarded-For, X-Real-IP) sont
//!   ignorés pour cette décision de sécurité — ils ne peuvent pas accorder de bypass.
//!
...
///
/// Le peer_addr est lu depuis l'extension `ConnectInfo<SocketAddr>` injectée par
/// `into_make_service_with_connect_info::<SocketAddr>()` au démarrage du serveur.
/// Si l'extension est absente (tests mock sans ConnectInfo), le bypass loopback
/// est refusé par défaut (sécurité conservative).
///
/// # Effets de bord
/// - Lit le header `Authorization` et l'extension `ConnectInfo` — ne modifie pas la requête.
...
pub async fn bearer_auth(
    ...
    // Bypass loopback : s'appuie sur le peer_addr socket TCP réel.
    // L'extension ConnectInfo est injectée par into_make_service_with_connect_info.
    // SÉCURITÉ : les headers X-Forwarded-For / X-Real-IP sont délibérément ignorés —
    // un client distant peut les forger librement. Seul le noyau connaît le vrai peer_addr.
    if state.trust_localhost {
        ...
        // Si ConnectInfo absent (transport mocké sans vrai socket) → refus par défaut.
    }
    ...
    match provided {
        // Comparaison en temps constant — protège contre les timing oracles.
        // expose_secret() dans un scope court : les bytes exposés ne survivent pas au match arm.
