...
> Async SQLite queue store (`SqliteQueueStore`) — Apalis-backed job queue for gradatum-worker.

**Status**: Alpha (v0.4.x) — internal (publish = false). API not yet stable before v1.0.
Part of **[gradatum](https://crates.io/crates/gradatum)** — memory backbone for AI agents. · [github](https://github.com/gradatum/gradatum) · [gradatum.org](https://gradatum.org)

## Overview

`gradatum-db-sqlite` provides the concrete SQLite-backed `SqliteQueueStore` used by
`gradatum-worker` and `gradatum-server` as the Apalis job queue backend.
...
```toml
[dependencies]
gradatum-db-sqlite = { path = "../gradatum-db-sqlite", version = "0.4.3" }
```
