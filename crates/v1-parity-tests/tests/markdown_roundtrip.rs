//! v1-parity : Markdown round-trip — 3 tests (D5 / spec §0.4 A3)
//!
...
///
/// Phase 2+ : bloqué par le design Phase 1 — `ExtraFields` est un champ nommé
/// explicite dans le YAML (`extra: {key: value}`), pas un catchall automatique
/// pour les champs inconnus au top-level. Les champs inconnus (ex. `source_tool`)
/// sont ignorés silencieusement par serde_yml (pas de `deny_unknown_fields`).
/// L'implémentation complète B8 nécessite un `#[serde(flatten)]` custom
/// ou un parser YAML two-pass — prévu Phase 2+.
#[tokio::test]
