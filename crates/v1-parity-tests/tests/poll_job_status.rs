...
/// E2E : vault_classify → 501 Not Implemented (non câblé en v0.4.x).
///
/// La classification est effectuée automatiquement lors du `vault_write`.
/// Un endpoint dédié est prévu dans une version ultérieure.
#[tokio::test]
async fn vault_classify_returns_501_not_implemented() {
    let (addr, bearer) = spawn_write_server().await;
    ...
    assert_eq!(
        resp.status(),
        501,
        "vault_classify doit retourner 501 Not Implemented en v0.4.x"
    );

    let body: Value = resp.json().await.expect("vault_classify 501 body JSON");
    assert_eq!(
        body["error"].as_str(),
        Some("vault_classify not implemented in v0.4.x"),
        "champ error attendu"
    );
    assert!(body["hint"].as_str().is_some(), "champ hint présent");
}

/// E2E : vault_downgrade synchrone — 404 pour une note inexistante.
///
/// `vault_downgrade` est synchrone (200/404), non async 202.
/// L'handler opère directement sur l'index SQLite.
/// Une note inexistante retourne 404 Not Found.
#[tokio::test]
