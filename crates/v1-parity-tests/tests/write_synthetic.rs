...
//!
//! Deferred Phase 2.1 : ces tests nécessitent wiremock + leader election Dispatcher,
//! composants non disponibles en alpha.4. Marqués `#[ignore]` avec justification.
//!
...
/// Le worker (heuristique) assignera "decisions" en Phase 2.1 full E2E.
#[tokio::test]
...
///
/// Phase 1.2 : vault_write retourne des ULID (non ordonnés numériquement).
...
/// plus async 202. Une note inexistante retourne directement 404 sans enqueue.
...
/// Test 20 — vault_classify (restore) enqueue OK.
///
/// Représente le "restore" : re-classification d'une note vers une section active.
/// Le curator reroute la note vers la section appropriée.
#[tokio::test]
async fn test_20_classify_restore_enqueues_job() {
    let (addr, bearer, queue) = spawn_write_server().await;
    ...
    assert_eq!(
        resp.status(),
        202,
        "test 20 : vault_classify doit retourner 202"
    );
    let body: Value = resp.json().await.expect("body 202 classify");
    let job_id = body["job_id"].as_i64().expect("job_id classify");
    assert!(job_id > 0);

    let depth = queue.depth().await.expect("depth queue post-classify");
    assert_eq!(depth, 1, "test 20 : 1 job classify doit être en queue");
}
...
/// deferred Phase 2.1 — voir test_22b_leader_election_deferred_phase_2_1.
#[tokio::test]
...
/// Test 22b — leader election (deferred Phase 2.1).
///
