...
//! ## Périmètre actuel
//!
//! Ces tests vérifient la forme des réponses (shape parity), pas le contenu.
//! La comparaison de contenu (diff JSON nul via [`diff_json_strip_tenant`]) est
//! différée et nécessite `migrate-from-v0` (import legacy vault v1.6.2 → gradatum-storage).
//!
//! # Preset ACL de test
//!
...
/// # Usage futur
///
/// Cette fonction sera activée avec `migrate-from-v0` pour la
...
// ── Constantes scaffold pour parity contenu (conservées #[allow(dead_code)]) ──

/// Port d'écoute de l'instance legacy vault v1.6.2 de test.
#[allow(dead_code)]
const LEGACY_VAULT_TEST_PORT: u16 = 18462;

/// Port d'écoute de l'instance gradatum-server de test.
#[allow(dead_code)]
const GRADATUM_TEST_PORT: u16 = 19190;

/// Chemin absolu vers le snapshot DB legacy vault v1.6.2.
#[allow(dead_code)]
...
///
/// vault_search retourne une liste paginée : champ `items` présent de type array
...
/// Shape parity : vault_read — retourne 404 si la note est absente.
///
/// vault_read retourne 404 pour toute note inexistante dans le vault de test (vide).
#[tokio::test]
async fn shape_vault_read() {
    ...
    // 404 attendu : le vault de test est vide, aucune note n'existe à ce chemin.
    assert_eq!(
        resp.status(),
        404,
        "vault_read doit retourner 404 pour une note absente"
    );
}
