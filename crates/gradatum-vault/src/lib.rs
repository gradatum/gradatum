...
pub trait Registry: Send + Sync {
    ...
    /// Maps `VaultError::Conflict` to `GradatumError::Storage("conflict: ...")` for
    /// callers that use the `Registry` trait (internal handlers).
    ///
    /// ## Errors
    ///
    /// - `GradatumError::Storage("conflict: hash mismatch")` if the note exists and
    ///   the hash does not match the expected value (optimistic-lock).
    /// - `GradatumError::Storage` / `GradatumError::Markdown` on I/O error.
...
impl Registry for Vault {
    ...
    async fn delete_note_by_id(
        ...
        self.delete_note(id).await.map_err(|e| match e {
            ...
            crate::error::VaultError::Conflict(hash) => GradatumError::Storage(format!(
                "delete_note_by_id : conflit inattendu hash={:?}",
                hash
            )),
        })
    }
