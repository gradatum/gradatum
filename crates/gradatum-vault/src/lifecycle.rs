...
impl Vault {
    ...
    /// ## Operations (in order)
    ///
    /// 1. Locates the `.md` on disk (without locus, then with section as locus).
    ///    If the note is absent from the index → returns `VaultError::Core(NoteNotFound)`.
    /// 2. Deletes the `.md` file via `storage.delete(md_path)`.
    /// 3. Purges the `.history/<id>/` directory via `purge_history_dir` (non-fatal).
    ///
    /// ## Error behaviour
    ///
