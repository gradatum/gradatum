...
//! - `delete_note` supprime le `.md` au bon chemin disque (locus vs sans locus).
...
/// `delete_note` supprime le `.md` au bon chemin disque lorsque la note a un locus.
///
/// Régression FIX 1 : le bug utilisait `record.section` au lieu de `record.locus`
/// pour construire le chemin de suppression. Ce test vérifie que le fichier physique
/// `main/my-section/<id>.md` est bien supprimé après `delete_note`.
#[tokio::test]
async fn d1_delete_note_with_locus_removes_md() {
    let dir = TempDir::new().unwrap();
    let vault = Vault::create(dir.path(), VaultId::new("main"))
        .await
        .unwrap();
    ...
    // Frontmatter avec locus — le .md atterrit dans main/my-section/<id>.md
    let mut fm_with_locus = build_minimal_frontmatter();
    fm_with_locus.locus = Some(LocusId::new("my-section"));

    vault
        .write_note_with_id(fm_with_locus, "contenu locus".into(), id)
        .await
        .unwrap();

    // Vérifier que le fichier existe bien sur disque AVANT la suppression.
    let md_path = dir
        .path()
        .join("main")
        .join("my-section")
    ...
    assert!(
        md_path.exists(),
        "le fichier .md doit exister avant delete_note : {}",
        md_path.display()
    );

    // Supprimer via delete_note.
    vault.delete_note(id).await.unwrap();

    // Le fichier .md NE DOIT PLUS EXISTER sur disque.
    assert!(
        !md_path.exists(),
        "le fichier .md doit être supprimé après delete_note : {}",
        md_path.display()
    );
}
...
/// Régression F1 — `delete_note` doit supprimer une note sans locus (chemin racine tenant).
///
/// Une note avec `locus=None` est écrite dans `<vault>/<id>.md` (layout racine, sans section
/// dans le chemin). Le bug régressif traitait `locus=None` comme "aucun chemin locus valide"
/// puis tombait directement dans `path_with_section` sans vérifier `path_no_locus` d'abord,
/// ce qui rendait ces notes INDÉLÉTABLES si la version section-as-locus n'existait pas.
///
/// Ce test vérifie que la résolution 3-voies trouve correctement `path_no_locus`
/// (`<vault>/<id>.md`) et supprime le fichier même quand `record.locus = None`.
#[tokio::test]
async fn f1_delete_note_legacy_no_locus_removes_md() {
    let dir = TempDir::new().unwrap();
    let vault = Vault::create(dir.path(), VaultId::new("main"))
        .await
        .unwrap();
    ...
    // Créer une note sans locus — `write_note_with_id` avec locus=None place le .md
    // dans `main/<id>.md` (chemin racine, produit par `note_md_relative_path` pour locus=None).
    let fm_no_locus = build_minimal_frontmatter();
    assert!(
        fm_no_locus.locus.is_none(),
        "le frontmatter de test ne doit pas avoir de locus"
    );

    vault
        .write_note_with_id(fm_no_locus, "note sans locus".into(), id)
        .await
        .unwrap();

    // Vérifier que le fichier existe bien sur disque dans le layout racine (`main/<id>.md`).
    // Note: locus=None → chemin `<tenant>/<id>.md`, PAS `<tenant>/<section>/<id>.md`.
    let md_path = dir.path().join("main").join(format!("{}.md", id));
    assert!(
        md_path.exists(),
        "le fichier .md (layout racine) doit exister avant delete_note : {}",
        md_path.display()
    );

    // Supprimer via delete_note — la résolution 3-voies doit trouver `path_no_locus`.
    vault
        .delete_note(id)
        .await
        .expect("delete_note doit réussir pour une note sans locus (chemin racine)");

    // Le fichier .md NE DOIT PLUS EXISTER sur disque.
    assert!(
        !md_path.exists(),
        "le fichier .md racine doit être supprimé après delete_note : {}",
        md_path.display()
    );
}
