...
///
/// Calls the 4 queue maintenance operations:
...
/// 4. [`idempotency_cleanup`] — purges idempotency entries older than 24 hours (TTL)
///
/// The `pool` is required to clean the `gradatum_idempotency` table (migration 008).
/// If `pool` is `None`, operation 4 is skipped with a WARN (the table may grow
