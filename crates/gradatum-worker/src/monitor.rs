...
use super::apalis_handlers::{
    handle_curate, handle_distill, handle_embed, handle_forget, handle_purge, handle_reindex,
    DistillSynthesizer,
};
...
pub struct MonitorDeps {
    /// Vault registry — note reads and writes (curate + embed).
    pub vault: Arc<Vault>,
    ...
    /// Index — wikilinks, reindex, and lifecycle (all workers).
    ///
    /// Type-erased `Arc<dyn Index>` — unified composition root with the server.
    /// The binary injects `Arc<SqliteIndex>` coerced to `Arc<dyn Index>`.
...
pub fn build_monitor(
    ...
    let mut monitor = Monitor::new()
        ...
        .register({
            ...
            let ack = purge_ack;
            let m = m_purge;
            let vault_p = Arc::clone(&vault);
            let index_p = Arc::clone(&index);
            move |idx| {
                let worker_name = format!("purge-{idx}");
                WorkerBuilder::new(&worker_name)
                    .backend(backend.clone())
                    .data(Arc::clone(&vault_p))
                    .data(Arc::clone(&index_p))
                    .ack_with(ack.clone())
                    .enable_tracing()
                    .timeout(Duration::from_secs(cfg.timeout_secs))
                    .retry(RetryPolicy::retries(cfg.max_retries))
        ...
        .register({
            ...
            let ack = forget_ack;
            let m = m_forget;
            let vault_f = Arc::clone(&vault);
            let index_f = Arc::clone(&index);
            move |idx| {
                let worker_name = format!("forget-{idx}");
                WorkerBuilder::new(&worker_name)
                    .backend(backend.clone())
                    .data(Arc::clone(&vault_f))
                    .data(Arc::clone(&index_f))
                    .ack_with(ack.clone())
                    .enable_tracing()
                    .timeout(Duration::from_secs(cfg.timeout_secs))
                    .retry(RetryPolicy::retries(cfg.max_retries))
        ...
        .register({
            ...
            move |idx| {
                ...
                WorkerBuilder::new(&worker_name)
                    ...
                    .data(Arc::clone(&vault_d))
                    .data(Arc::clone(&index_d))
