...
impl gradatum_core::IndexStore for FaultStatusIndex {
    ...
    async fn id_lookup(
        ...
        self.inner.id_lookup(vault_id, note_id).await
    }
    ...
    async fn code_scope_reverse_deps(
        ...
        qualified_name: &str,
        limit: usize,
    ) -> Result<Vec<gradatum_core::index_store::CodeScopeEntryRaw>, GradatumError> {
        self.inner
            .code_scope_reverse_deps(vault_id, qualified_name, limit)
            .await
    }
}
...
/// handle_purge mode réel : une note candidate dont `get_note` retourne une erreur
/// (simule un statut hors-enum / note illisible pendant la boucle) NE DOIT PAS
...
async fn purge_tolerates_unparseable_status_and_continues_batch() {
    ...
    /// Wraps `TestInternalClient` but injects a `ServerError` on `get_note` for a
    /// designated note id — simulates TOCTOU status parse failure mid-batch.
    struct FaultGetNoteClient {
        inner: TestInternalClient,
        fault_id: String,
    }

    #[async_trait]
    impl InternalClient for FaultGetNoteClient {
        async fn persist_curated(
            &self,
            req: &PersistCuratedRequest,
        ) -> Result<PersistOkResponse, InternalClientError> {
            self.inner.persist_curated(req).await
        }
        async fn persist_embedding(
            &self,
            req: &PersistEmbeddingRequest,
        ) -> Result<EmbeddingOkResponse, InternalClientError> {
            self.inner.persist_embedding(req).await
        }
        async fn persist_forget(
            &self,
            req: &PersistForgetRequest,
        ) -> Result<PersistOkResponse, InternalClientError> {
            self.inner.persist_forget(req).await
        }
        ...
        async fn get_note_status(
            ...
            if ulid == self.fault_id {
                return Err(InternalClientError::ServerError {
                    status: 500,
                    body: format!(
                        "parse NoteStatus 'downgraded' : unknown variant (fault injecté) pour {ulid}"
                    ),
                });
            }
        ...
        async fn title_lookup(
            ...
            self.inner.title_lookup(tenant, title).await
        }
        async fn id_lookup(
            &self,
            tenant: &str,
            ...
            self.inner.id_lookup(tenant, note_id).await
        }
        async fn list_notes_by_locus(
            ...
            self.inner.list_notes_by_locus(vault, prefix).await
        }
        async fn list_by_status(
            &self,
            vault: &str,
            ...
            self.inner.list_by_status(vault, status).await
        }
        async fn list_garbage(
            ...
            self.inner.list_garbage(vault, before_ms, grace_days).await
        }
        async fn search_fts_for_forget(
            &self,
            vault: &str,
            ...
            self.inner.search_fts_for_forget(vault, query, limit).await
        }
        async fn list_notes_by_agent(
            &self,
            agent: &str,
            ...
            self.inner.list_notes_by_agent(agent, vaults).await
        }
    }
