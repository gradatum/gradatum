...
use chrono::Utc;
use gradatum_core::{
    scope::VaultId, CurateSpec, GradatumJob, Job, JobClass, JobLifecycle, JobLineage, JobMode,
...
async fn curate_persists_title_in_index() {
    ...
    let hits = fixture
        .index
        .search_fts_with_snippet(&vault_id, "Régression Persist", 5, false, None)
        .await
...
async fn search_fts_returns_title_after_curate() {
    ...
    let hits = fixture
        .index
        .search_fts_with_snippet(&vault_id, "correction bug", 5, false, None)
        .await
        .expect("search_fts_with_snippet");
