...
use gradatum_worker::apalis_handlers::{
    handle_distill, ClusterSynthesis, DistillSynthesizer, SynthesisError, TemplateSynthesizer,
};
...
async fn dry_run_lists_clusters_without_mutation() {
    ...
    let out = handle_distill(
        job,
        Data::new(Arc::clone(&fx.vault)),
        Data::new(Arc::clone(&fx.index) as std::sync::Arc<dyn gradatum_core::index::Index>),
