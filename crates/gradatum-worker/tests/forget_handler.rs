...
use chrono::Utc;
use gradatum_core::{
    scope::VaultId, ForgetScope, ForgetSpec, GradatumJob, Job, JobClass, JobLifecycle, JobLineage,
...
async fn fail_closed_note_not_in_index_excluded() {
    ...
    let out = handle_forget(
        ...
        Data::new(fixture.index.clone() as std::sync::Arc<dyn gradatum_core::index::Index>),
    )
    .await
