...
impl CuratorPipeline {
    ...
    pub async fn process(&self, note: Note) -> CurateOutcome {
        ...
        // l'une des 11 sections canoniques, on admet directement sans consulter
        // l'heuristique ni le LLM. Un hint invalide est ignoré avec un warn.
        if let Some(ref hint) = note.section_hint {
            if routing::SECTIONS.contains(&hint.as_str()) {
                tracing::debug!(
                    title = %note.title,
                    section = %hint,
        ...
        // retourne directement tout hint valide parmi les 11 sections canoniques,
