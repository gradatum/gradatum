...
/// Chemin profond inexistant (plusieurs niveaux d'imbrication absents).
///
/// Régression FIX H : l'ancienne impl ne remontait que d'un niveau (parent immédiat),
/// échouant si `nouveau_vault/` n'existait pas non plus.
/// La nouvelle impl remonte la chaîne jusqu'au premier ancêtre existant.
#[test]
fn deeply_nested_nonexistent_path_with_local_ancestor_returns_ok() {
    let dir = TempDir::new().expect("TempDir::new() ne doit pas échouer");
    // Trois niveaux d'imbrication absents — ni `a/`, ni `a/b/`, ni `a/b/c/` n'existent.
    let deep_path = dir.path().join("a").join("b").join("c");
    let result = ensure_local_filesystem(&deep_path);
    assert!(
        result.is_ok(),
        "ensure_local_filesystem sur chemin profond inexistant doit remonter jusqu'à l'ancêtre local, obtenu : {result:?}"
    );
}
