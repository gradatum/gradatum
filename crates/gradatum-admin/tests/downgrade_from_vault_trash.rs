//! Phase 2.1.2 alpha.9 — Tests intégration `downgrade-from-legacy-vault-trash`.
//!
//! Stratégie :
...
use gradatum_admin::{downgrade_from_vault_trash, DowngradeFromTrashArgs};
...
///
/// Reproduit le bug smoke alpha-9 Phase 14 : le tool retournait Err quand le
/// répertoire `.vault-trash` avait été nettoyé (300 fichiers .bck supprimés
/// 2026-05-09). `set -euo pipefail` dans le smoke abortait silencieusement.
