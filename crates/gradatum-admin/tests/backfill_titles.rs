//! Tests d'intégration TDD — sub-commande `backfill-titles` (Task 21 alpha.15).
//!
//! Stratégie :
...
///
/// Insère avec un title calculé (comportement post-alpha.10), puis force
...
/// `title = ""`.
///
/// `title = ""` est sémantiquement distinct de `title = NULL` : un second
/// passage du backfill ne retraiterait pas WHERE title IS NULL → régression
...
async fn backfill_titles_empty_h1_is_skipped() {
    ...
    {
        let conn = rusqlite::Connection::open(&index_path).expect("open index");
        // H1 vide après strip : "# " → extract_h1_title retourne Some("").
