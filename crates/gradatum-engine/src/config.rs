...
pub struct EngineConfig {
    ...
    ///   `{url}/api/v1/event-log`. The URL must pass `validate_loopback_url`
    ///   (anti-SSRF) — loopback or validated unicast.
    ...
    /// (`/usr/local/bin/` or `/opt/gradatum/bin/`) at supervisor construction
    /// (canonicalization prevents TOCTOU).
...
impl EngineConfig {
    ...
    /// (`/api/v1/config/:binary`) is a deferred figment provider (not yet implemented).
    ///
