...
//! correctement parsés, et que les fonctions stubs Phase 1 retournent `Ok`.

use gradatum_core::schema_registry::{lookup, SchemaStatus};

/// metadata-v1 est le schéma Phase 1 stable — tous ses champs doivent être présents.
#[test]
...
/// acl-v1 est un stub Phase 2 — vérifie que le schéma est bien chargé et marqué experimental.
#[test]
...
/// `validate_payload` Phase 1 accepte n'importe quoi (stub).
#[test]
fn validate_payload_phase1_stub_accepts_anything() {
    ...
    assert!(
        result.is_ok(),
        "validate_payload Phase 1 doit toujours retourner Ok"
    );
    ...
    assert!(
        result.is_ok(),
        "validate_payload Phase 1 n'échoue jamais (stub)"
    );
}

/// `migrate_payload` Phase 1 retourne le payload inchangé (stub).
#[test]
