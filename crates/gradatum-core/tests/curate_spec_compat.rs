//! Tests de régression serde JSON pour [`CurateSpec`].
//!
//! Vérifie la compatibilité forward/backward entre les payloads minimaux
//! (champs `note_id` + `tenant_id` seuls) et les payloads complets
//! (champs étendus `title`/`body`/`author`/`tags`/`section_hint`).
//!
//! # Contexte
//!
//! `CurateSpec` a été étendu avec 5 champs optionnels annotés `#[serde(default)]`.
//! Les `JobRecord` stockés en base avec le payload minimal n'ont PAS ces champs
//! en JSON — ils doivent se désérialiser en `None`/`[]` (valeurs par défaut) sans erreur.
...
/// Simule un payload JSON minimal (note_id + tenant_id uniquement).
///
...
/// Un `Job::Curate` wrappant un `CurateSpec` minimal se désérialise correctement.
///
...
fn job_curate_with_alpha15_spec_deserializes() {
    ...
    match job {
        Job::Curate(spec) => {
            ...
            assert!(
                spec.title.is_none(),
                "title doit être None (champ absent du payload)"
            );
            assert!(
                spec.body.is_none(),
                "body doit être None (champ absent du payload)"
            );
            assert!(
                spec.tags.is_empty(),
                "tags doit être [] (champ absent du payload)"
            );
        }
