...
/// ## Panics
///
/// Never panics in production. `serde_jcs::to_string` can only fail if
/// `Frontmatter` contains a non-serialisable float (`f32::NAN`). `Frontmatter`
/// contains no `f32` fields → the `expect` is justified by the same invariant as
/// `ContentHash::compute`.
