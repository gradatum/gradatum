...
//! # Invariant
//!
//! Si l'un de ces tests échoue après le carve 3 traits (Étape 0.1), le carve a modifié
...
async fn golden_bm25_order_is_stable() {
    ...
    let hits = idx
        .search_fts_with_snippet(&vault, "gradatum", 10, false, None)
        .await
        .unwrap();
...
async fn golden_semantic_order_is_stable() {
    ...
    let results = idx
        .search_semantic("main", "golden-embedder", &[1.0f32, 0.0, 0.0], 3)
        .await
        .unwrap();
