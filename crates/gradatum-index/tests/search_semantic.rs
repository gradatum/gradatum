//! Tests intégration `SqliteIndex::search_semantic` — Phase 2.x.2 alpha.11.
//!
//! Couvre :
...
async fn search_semantic_returns_top_k_by_cosine() {
    ...
    let results = idx
        .search_semantic("main", "test-embedder", &query_emb, 3)
        .await
        .unwrap();
...
async fn search_semantic_query_zero_norm_returns_empty() {
    ...
    let results = idx
        .search_semantic("main", "test-embedder", &zero_query, 10)
        .await
        .unwrap();
...
async fn search_semantic_ignores_other_embedder_id() {
    ...
    let results = idx
        .search_semantic("main", "embedder-B", &query, 10)
        .await
        .unwrap();
...
async fn search_semantic_excludes_downgraded_notes() {
    ...
    let results = idx
        .search_semantic("main", "test-embedder", &query, 10)
        .await
        .unwrap();
...
async fn search_semantic_limit_truncates_results() {
    ...
    let results = idx
        .search_semantic("main", "test-embedder", &query, 3)
        .await
        .unwrap();
...
async fn search_semantic_perf_n1500_p95_under_200ms() {
    ...
    for _ in 0..runs {
        let t0 = Instant::now();
        let results = idx
            .search_semantic("main", embedder_id, &query_emb, 10)
            .await
            .unwrap();
