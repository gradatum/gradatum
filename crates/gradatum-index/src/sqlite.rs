        ...
        let collected: Vec<RawRow> = match (section, locus) {
            ...
            (None, Some(loc)) => {
                ...
                let result = stmt
                    ...
                    .map_err(|e| {
                        GradatumError::Storage(format!(
                            "query search_fts_scored_filtered (locus): {e}"
                        ))
                    })?
                    .collect::<rusqlite::Result<Vec<_>>>()
                    .map_err(|e| {
        ...
        let raw_rows: Vec<RawRow> = match (section, locus) {
            ...
            (None, Some(loc)) => {
                ...
                let result = stmt
                    ...
                    .map_err(|e| {
                        GradatumError::Storage(format!(
                            "query search_fts_with_snippet (locus): {e}"
                        ))
                    })?
                    .collect::<rusqlite::Result<Vec<_>>>()
                    .map_err(|e| {
                        GradatumError::Storage(format!(
                            "collect search_fts_with_snippet (locus): {e}"
                        ))
                    })?;
                result
            }
    ...
    pub(crate) async fn search_semantic_inner(
        ...
        let raw_rows: Vec<SemRow> = if let Some(loc) = locus {
            ...
            let result = stmt
                ...
                .collect::<rusqlite::Result<Vec<_>>>()
                .map_err(|e| {
                    GradatumError::Storage(format!("search_semantic collect (locus): {e}"))
                })?;
            result
        } else {
            let mut stmt = conn
...
impl SqliteIndex {
    ...
    pub async fn list_forgotten_notes(
        ...
        let rows: Vec<ForgottenRow> = if let Some(cur) = cursor {
            ...
            let result = stmt
                .query_map(rusqlite::params![vault_id, cur, fetch_limit_i64], |row| {
                    Ok((
                        row.get::<_, String>(0)?,
                        row.get::<_, Option<String>>(1)?,
                        row.get::<_, String>(2)?,
                ...
                .map_err(|e| {
                    GradatumError::Storage(format!("query list_forgotten_notes cursor: {e}"))
                })?
            ...
            let result = stmt
                .query_map(rusqlite::params![vault_id, fetch_limit_i64], |row| {
                    Ok((
                        row.get::<_, String>(0)?,
                        row.get::<_, Option<String>>(1)?,
                        row.get::<_, String>(2)?,
                ...
                .collect::<Result<Vec<_>, _>>()
                .map_err(|e| {
                    GradatumError::Storage(format!("collect list_forgotten_notes: {e}"))
                })?;
            result
        };
        Ok(rows)
    ...
    pub async fn timeline(
        ...
        let mut binds: Vec<SqlVal> = vec![SqlVal::Text(vault_id.0.clone())];
        binds.extend(
