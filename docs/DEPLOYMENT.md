# Engine Deployment Guide — Multi-instance

> Platform: **Linux only** (x86_64 and aarch64). gradatum does not support Windows or macOS.

This guide covers deploying `gradatum-engine` in a single-host or multi-instance setup.
Each instance supervises one `llama-server` child process and exposes an OpenAI-compatible
HTTP API. Instances run independently and can be wired into `gradatum-gateway` for load
balancing, failover, and circuit-breaking.
