...
> Updated: 2026-05-08 T6 P2.0c-tris commit `b6249c0` — `gradatum-curator`: ajout dev-deps `wiremock` (mock HTTP server tests LLM integration) + `secrecy` (secret string handling tests). `secrecy` déjà en [dependencies] depuis T1 (share across crate + tests). Root cause fix fallback_on_error : CircuitBreaker retiré du from_config(), backend brut → error propagation directe tests.
> Updated: 2026-05-07 P2.0c-bis alpha.5 chunks 1+2 commit `6f76a8e` — `gradatum-acl-auth`: activation 10 workspace deps (`argon2`, `async-trait`, `rand`, `serde[derive]`, `serde_json`, `sqlx[sqlite,runtime-tokio,macros]`, `thiserror`, `tokio[full]`, `tracing`, `ulid[serde]`) + dev-deps (`tokio[test-util]`, `tempfile`). `gradatum-admin`: ajout dep workspace `gradatum-auth` + dev-dep `rusqlite`. Aucune nouvelle dep externe au workspace.
> Updated: 2026-05-05 T13 Phase A P2.0c — `gradatum-server`: ajout `sd-notify = "0.4"` sous `[target.'cfg(target_os = "linux")'.dependencies]` (sd_notify::Ready post-bind). T9 P2.0c — `gradatum-worker` promu `[workspace.dependencies]` + dev-dep `gradatum-server`. Sprint X-1 2026-05-04 — `gradatum-storage`: `nix` déplacé sous `[target.'cfg(unix)'.dependencies]`. T01: workspace deps pinnées.
