...
- **gradatum-studio** (new crate, `publish = false`) : React+TS+Vite admin UI (5 surfaces / 6 routes), served by gradatum-server via `tower-http` ServeDir on `/ui/*` (SPA fallback, CSP + security headers — `gradatum-server/src/studio.rs`). Auth: api-key → JWT (sessionStorage client-side). Bundle deployed to `/usr/share/gradatum/ui` (configurable `[studio] ui_dir`).
